...
enum GraphLoader {
    /// Load SoT for a project: prefer `.swiftprism/prism-context.json` (v2 context or flat nodes).
    static func load(projectRoot: URL) throws -> GraphDocument {
        let sot = projectRoot.appendingPathComponent(".swiftprism/prism-context.json")
