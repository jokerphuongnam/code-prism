    ...
    var body: some View {
        ...
        .toolbar {
            ToolbarItemGroup {
                ...
                Divider()
                Button(model.backendInstalled ? "Reinstall backend" : "Install backend") {
                    model.installBackend()
                }
        ...
        ZStack {
            if model.document.nodes.isEmpty {
                ContentUnavailableView(
                    ...
                    description: Text("Open a project, install the Swift backend plugin, then Analyze to write SoT under .swiftprism/.")
                )
            } else {
                GraphSceneView(
