...
function extractSourceCode(node, maxLines = 300) {
...
// OLLAMA CLIENT
// ═══════════════════════════════════════════════════════════════════════════════
async function queryOllama(systemPrompt, userPrompt, config) {
...
function extractTokens(contexts, prefix) {
    const results = [];
    const seen = new Set();
...
function inferFunctionIntent(name) {
    ...
    if (/^(get|fetch|load|read|query|find|search)/.test(n))
        return "read";
    if (/^(set|update|save|write|store|put|patch)/.test(n))
        return "write";
    if (/^(create|make|build|init|new|generate)/.test(n))
        return "create";
    if (/^(delete|remove|drop|destroy|clear)/.test(n))
        return "delete";
    if (/^(render|draw|display|show|present)/.test(n))
        return "render";
    if (/^(validate|check|verify|assert|ensure)/.test(n))
        return "validate";
    if (/^(configure|setup|register|bind)/.test(n))
        return "setup";
    if (/^(handle|on[A-Z]|did|will|process)/.test(n))
        return "handle";
    if (/^(parse|decode|encode|serialize|transform|convert)/.test(n))
        return "transform";
    return null;
}
function inferStructPurpose(name) {
    const n = name.toLowerCase();
    if (/config|setting|option/.test(n))
        return "configuration";
    if (/request|response|payload/.test(n))
        return "data_transfer";
    if (/model|entity|data|info/.test(n))
        return "data_model";
    if (/state/.test(n))
...
function inferEnumPurpose(name) {
    const n = name.toLowerCase();
    if (/route|screen|destination|tab/.test(n))
        return "navigation";
    if (/state|status|phase/.test(n))
        return "state_machine";
    if (/error|failure/.test(n))
        return "error_cases";
    if (/action|event|command/.test(n))
        return "action_dispatch";
    if (/type|kind|category|style/.test(n))
        return "classification";
    return null;
}
/** Analyze init/deinit lifecycle for an object node. */
function analyzeLifecycle(node, sourceCode) {
    let ini = null;
    let di = null;
    // Init analysis
    ...
    if (hasInits || sourceCode) {
        ...
        if (sourceCode && /init\s*\(/.test(sourceCode)) {
            if (traits.length === 0)
                traits.push("assign_only");
            if (/addObserver|\.observe\(/.test(sourceCode))
                traits.push("add_observers");
            if (/URLSession|connect|\.start\(\)/i.test(sourceCode))
                traits.push("conn_remote");
            if (/Timer\.|schedule/i.test(sourceCode))
                traits.push("start_timer");
            if (/super\.init/.test(sourceCode))
                traits.push("calls_super");
        }
        const unique = [...new Set(traits)];
        if (unique.length > 0)
            ini = unique.slice(0, 4).join(",");
    }
    // Deinit analysis (reference types only)
    if (node.flavor === "class" || node.flavor === "actor") {
        const traits = [];
        if (node.deinits?.length)
            traits.push(...node.deinits.slice(0, 4).map(leafName));
        if (sourceCode && /deinit\s*\{/.test(sourceCode)) {
            if (/cancel\(\)|\.cancel\b/i.test(sourceCode))
                traits.push("cancel_tasks");
            if (/removeObserver/i.test(sourceCode))
                traits.push("remove_observers");
            if (/close\(\)|disconnect/i.test(sourceCode))
                traits.push("close_conn");
            if (/invalidate\(\)/i.test(sourceCode))
                traits.push("stop_timer");
            if (traits.length === 0)
                traits.push("cleanup");
        }
        const unique = [...new Set(traits)];
        if (unique.length > 0)
            di = unique.slice(0, 4).join(",");
    }
    return { ini, di };
}
/** Determine the executable sub-tag. Returns null for type-level nodes. */
function inferExecTag(node) {
    const name = node.name.toLowerCase();
    if (node.flavor === "variable") {
        if (name === "willset" || name.endsWith(".willset"))
            return "ws";
        if (name === "didset" || name.endsWith(".didset"))
            return "ds";
        if (name === "getter" || name.endsWith(".get"))
            return "g";
        if (name === "setter" || name.endsWith(".set"))
            return "st";
...
function inferExtensionCapabilities(mergedSource) {
    ...
    const caps = [];
    if (/Codable|Decodable|Encodable/.test(extSource))
        caps.push("coding");
    if (/Equatable|Hashable/.test(extSource))
        caps.push("equality");
    if (/CustomStringConvertible/.test(extSource))
        caps.push("debug_description");
    if (/tableView|collectionView|UITableView/.test(extSource))
        caps.push("table_data_source");
    if (/@objc|#selector|@IBAction/.test(extSource))
        caps.push("objc_actions");
    if (/\.snp\.|makeConstraints|NSLayoutConstraint/.test(extSource))
        caps.push("layout");
    if (/style|theme|backgroundColor/.test(extSource))
        caps.push("styling");
    if (/@Published|@State|Combine/.test(extSource))
        caps.push("reactive");
    if (/PreviewProvider|#Preview/.test(extSource))
        caps.push("preview");
    if (caps.length === 0) {
...
function generateFallbackContext(node, sourceCode, childContexts, callerNames, extensionCount = 0) {
    const parts = [];
    ...
    if (node.flavor === "target") {
        if (!node.origin)
            parts.push("i:internal_module");
        else if (node.origin === "Apple")
            parts.push(`i:${APPLE_ROLES[node.name] ?? "apple_sdk"}`);
        else
            parts.push("i:third_party");
    }
    else if (OBJECT_FLAVORS.has(node.flavor)) {
        if (childContexts.length > 0) {
            const intents = extractTokens(childContexts, "i:");
            if (intents.length > 0)
                parts.push(`i:${intents.slice(0, 3).join(",")}`);
        }
        else {
            const purpose = inferStructPurpose(node.name) ?? inferEnumPurpose(node.name);
            if (purpose)
                parts.push(`i:${purpose}`);
        }
    }
    else {
        // Executables — special-case accessors and lifecycle
        const lowName = node.name.toLowerCase();
        if (lowName === "willset" || lowName.endsWith(".willset"))
            parts.push("i:validate_before_set");
        else if (lowName === "didset" || lowName.endsWith(".didset"))
            parts.push("i:react_after_set");
        else if (lowName === "deinit")
            parts.push("i:cleanup");
        else if (node.flavor === "initializer" || lowName === "init" || lowName.startsWith("init("))
            parts.push("i:initialize");
        else {
    ...
    const structural = [];
    if (node.extends)
        structural.push(leafName(node.extends));
    if (node.implements?.length)
        for (const id of node.implements.slice(0, 3))
            structural.push(leafName(id));
    if (structural.length > 0)
        parts.push(`d:${structural.join(",")}`);
    const triggers = [];
    for (const id of (node.calls ?? []).slice(0, 4))
        triggers.push(leafName(id));
    if (node.flavor === "target" && callerNames.length > 0)
        triggers.push(...callerNames.slice(0, 4));
    if (triggers.length > 0)
        parts.push(`tr:${triggers.join(",")}`);
    ...
    if (OBJECT_FLAVORS.has(node.flavor)) {
        ...
        if (lifecycle.di)
            parts.push(`di:${lifecycle.di}!->@st:closed`);
        const accSummary = summarizeAccessors(childContexts);
        if (accSummary)
            parts.push(`acc:${accSummary}`);
        if (extensionCount > 0) {
            const extCaps = inferExtensionCapabilities(sourceCode);
            if (extCaps)
                parts.push(`ext:${extCaps}`);
        }
    }
    // Intelligence annotations
    if (sourceCode) {
        // err:
        if (/catch.*retry|retry.*catch/i.test(sourceCode))
            parts.push("err:retry");
        else if (/\bthrow\b|\bthrows\b/.test(sourceCode))
            parts.push("err:throw");
        else if (/try\?\s/.test(sourceCode))
            parts.push("err:silent");
        // sync:
        if (node.flavor === "actor")
            parts.push("sync:actor");
        else if (/DispatchQueue\.main|@MainActor/.test(sourceCode))
            parts.push("sync:main");
        else if (/DispatchQueue\.global|\.background/.test(sourceCode))
            parts.push("sync:bg");
        else if (/\basync\b|\bawait\b|Task\s*\{/.test(sourceCode))
            parts.push("sync:async");
        // cost:
        if (/for\s.*in\s.*for\s.*in/s.test(sourceCode))
            parts.push("cost:H");
        else
            parts.push("cost:L");
        // p: pattern
        if (/\.shared\b|static\s+let\s+\w+\s*[:=]\s*\w+\(\)/.test(sourceCode))
            parts.push("p:singleton");
        else if (/delegate\?\.|Delegate/.test(sourceCode))
            parts.push("p:delegate");
        else if (/NotificationCenter|@Published|\.sink\b/.test(sourceCode))
            parts.push("p:observer");
        // @ state effects
        const effects = [];
        if (/URLSession|URLRequest/i.test(sourceCode))
            effects.push("net");
        if (/FileManager/i.test(sourceCode))
            effects.push("disk");
        if (/UserDefaults|CoreData/i.test(sourceCode))
            effects.push("state");
        if (/UIView|SwiftUI/i.test(sourceCode))
            effects.push("ui");
        if (effects.length > 0)
    ...
    if (callCount >= 8)
        parts.push("hub");
    else if (callCount === 0 && !(node.stores?.length))
        parts.push("leaf");
    return parts.join("|");
}
...
export async function generateNodeContexts(graphPath, outputDir, configOverrides = {}) {
    ...
    const result = { total: 0, generated: 0, cached: 0, failed: 0, usedLLM: false };
    // Check Ollama
    ...
    const parents = [];
    for (const n of nodes) {
        if (!config.eligibleFlavors.has(n.flavor))
            continue;
        if (OBJECT_FLAVORS.has(n.flavor))
            parents.push(n);
        else
            leaves.push(n);
    }
    ...
    console.error(`[NodeContext] Done: ${result.generated} generated, ${result.cached} cached` +
        ` (LLM: ${result.usedLLM ? "yes" : "fallback"})`);
    return result;
}
function buildFileSummaries(nodes) {
    const byFile = new Map();
    for (const n of nodes) {
        if (n.flavor === "target" || !n.location.absPath)
            continue;
...
function buildTargetSummaries(nodes) {
    const summaries = {};
    for (const n of nodes) {
        if (n.flavor !== "target")
            continue;
        ...
        if (!existing && intents.length > 0)
            parts.push(`scope:${intents.slice(0, 4).join(",")}`);
        summaries[n.id] = parts.join("|");
    }
    return summaries;
}
async function processNode(node, allNodes, cache, config, ollamaAvailable, result, childrenOf, callersOf) {
    ...
    if (isParent) {
        for (const childId of childrenOf.get(node.id) ?? []) {
            ...
            if (child?.node_context)
                childContexts.push(child.node_context);
        }
    }
    ...
    if (!isTarget) {
        if (isParent && node.locations && node.locations.length > 0) {
            ...
            for (const loc of node.locations) {
                const extNode = { ...node, location: loc };
    ...
    if (isTarget) {
        ...
        if (node.inits?.length)
            hashInput += `|inits:${node.inits.join(",")}`;
        if (node.deinits?.length)
            hashInput += `|deinits:${node.deinits.join(",")}`;
    }
    ...
    if (ollamaAvailable) {
        if (isTarget) {
            ...
            if (node.origin)
                meta.push(`Origin: ${node.origin}`);
            if (callerNames.length > 0)
                meta.push(`Used by: ${callerNames.slice(0, 8).join(", ")}`);
